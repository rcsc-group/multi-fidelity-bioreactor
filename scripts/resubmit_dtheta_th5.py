"""Resubmission for theta_source=5 -> target=2 (diary.md 2026-09-07),
cancelled twice by CPU-cap contention, not a bug. Ample headroom now.

Usage:
    uv run python scripts/resubmit_dtheta_th5.py
"""
import json
import sys
...
theta_source = 5.0
src_dir = RUNS_DIR / "dtheta_src_L6_th5"
...
manifest["edges"]["5"] = {"theta_source": theta_source, "run_id": run_id,
                           "source_run": "dtheta_src_L6_th5"}
manifest_path.write_text(json.dumps(manifest, indent=2))
print(f"theta_source=5 -> target=2: run={run_id}  {job_run_ids}")
