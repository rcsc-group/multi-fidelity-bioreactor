...
analysis + plot (diary.md 2026-09-07). Same methodology as
analyze_dtheta_monotonicity.py, target=(32.5rpm, theta=2deg), sources
approaching from above (theta-decreasing direction -- matches the
direction of the ORIGINAL catastrophic anomaly, 7->4->2).

Usage:
    uv run python scripts/analyze_dtheta_monotonicity_decreasing.py
"""
import json
import math
...
RUNS = ROOT / "runs"
TOL = 0.05
MANIFEST = json.loads((ROOT / "experiments" / "dtheta_monotonicity_decreasing_manifest.json").read_text())
RPM = MANIFEST["rpm"]
...
baseline = np.loadtxt(RUNS / "settling_ref_L6_rpm32.5_th2" / "shear_stress.dat", skiprows=1)
...
# [FIX 2026-09-07, diary.md] A flat 5% relative tolerance was calibrated
# against the theta=7 target's own tail noise (0.69% std/mean) -- fine
# there, but theta=2's tau_mean is an order of magnitude smaller with much
# larger intrinsic cycle-to-cycle noise. A first attempt (3 sigma of the
# reference's own 5-cycle tail, ~2.5% std/mean -> 7.6% eff. tolerance) was
# still not enough: the trivial Delta_theta=0 identity-restart control
# (source==target, must settle at cycle 0 by construction) still came back
# as t_settle=56. A 5-cycle sample undersamples the true noise -- the
# trivial run itself (59 cycles, genuinely converged throughout since
# nothing was perturbed) shows single-cycle deviations from its own mean
# up to 11%. Calibrate the tolerance directly from that run instead: it is
# a matched-length (same window as every other transition), ground-truth
# "already converged" sample -- exactly what's needed.
_edges_raw = MANIFEST["edges"]
_trivial_run_id = _edges_raw["2"]["run_id"]
...
print(f"trivial-run (d_theta=0) noise calibration: max single-cycle deviation="
      f"{100*_max_reldev:.1f}%  -> effective tolerance={100*tol_eff:.1f}% (flat TOL={100*TOL:.0f}%)")
...
out_path = out_dir / "settling_vs_dtheta_decreasing.png"
