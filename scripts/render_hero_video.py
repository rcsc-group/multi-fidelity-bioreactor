...
(|u|, winter colormap), reconstructed from a real fresh L10 run's per-cell
dumps (diary.md 2026-08-28) rather than re-running the native OpenGL video
pipeline (docs_site/how-to/generate-videos.md) at L10, which would need a
new SLURM submission. Rotates the body-frame velocity field by Th(t) --
...
Went through several iterations on user feedback before this one: a flat
VOF two-tone render (too plain), then vorticity coloring (washed out --
vorticity in this laminar regime is confined to thin near-wall boundary
layers), then velocity magnitude with a "turbo" colormap (too visually
loud). This version uses "winter" instead and labels the field ("|u|")
...
def main():
    ...
    for fi, t in enumerate(times):
        ...
        ax.text(0.015, 0.93, "|u|", transform=ax.transAxes,
                fontsize=15, color="black", va="top", weight="bold")
        ax.text(0.985, 0.93, f"t = {t*T_bio:.2f} s", transform=ax.transAxes,
                fontsize=13, color="black", va="top", ha="right")
