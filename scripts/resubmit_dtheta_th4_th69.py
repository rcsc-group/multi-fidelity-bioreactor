"""Resubmit theta_source=6.9 -> target=4 (cancelled by QOSMaxCpuPerUserLimit,
diary.md 2026-09-07).

Usage:
    uv run python scripts/resubmit_dtheta_th4_th69.py
"""
import json
import sys
...
theta_source = 6.9
src_dir = RUNS_DIR / "dtheta_src_L6_th6.9"
...
manifest["edges"]["6.9"] = {"theta_source": theta_source, "run_id": run_id,
                             "source_run": "dtheta_src_L6_th6.9"}
manifest_path.write_text(json.dumps(manifest, indent=2))
print(f"theta_source=6.9 -> target=4: run={run_id}  {job_run_ids}")
