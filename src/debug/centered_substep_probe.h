/**
[PROJECT: TEMPORARY DIAGNOSTIC FORK, 2026-09-13] This is a deliberate,
disclosed, one-off fork of Basilisk's canonical
navier-stokes/centered.h (copied verbatim from
/oscar/data/dharri15/eaguerov/basilisk/src/navier-stokes/centered.h),
used ONLY to localize which per-timestep sub-step regenerates the
cross-level checkerboard artifact (diary.md 2026-09-13, seven fix
attempts all null/harmful). Four one-line probe calls are added at the
end of the advection_term/viscous_term/acceleration/projection event
bodies -- no other line is changed. Gated behind
-DCROSS_LEVEL_SUBSTEP_PROBE=1; BioReactor.c falls back to the real
canonical header otherwise. DELETE this file once the diagnostic is
done; it is not a replacement for the canonical header and must never
drift from it except for these four probe lines.
*/
...
{
  cross_level_substep_probe ("pre_advection");
  if (!stokes) {
    prediction();
    mgpf = project (uf, pf, alpha, dt/2., mgpf.nrelax);
    advection ((scalar *){u}, uf, dt, (scalar *){g});
  }
  cross_level_substep_probe ("post_advection");
}
...
event viscous_term (i++,last)
{
  ...
  cross_level_substep_probe ("post_viscous");
}
...
event acceleration (i++,last)
{
  ...
  cross_level_substep_probe ("post_acceleration");
}
...
event projection (i++,last)
{
  ...
  correction (dt);
  cross_level_substep_probe ("post_projection");
}
