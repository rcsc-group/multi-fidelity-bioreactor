...
| `08_l8_matrix_mpi_vs_openmp_vs_restart_video.mp4` | Our own 4 L8-matrix binaries (fresh MPI/OpenMP, restart-recovered MPI/OpenMP), \|u\| and τ, one shared colorbar per column, animated through each row's own dump sequence (240 frames, frame-index aligned -- fresh and restart start at different absolute t, so index alignment rather than time alignment is the fair comparison; each row carries its own `t=` label since the two pairs are on different clocks). Visual confirmation of diary.md 2026-08-21: MPI vs OpenMP rows track each other throughout; restart-recovered rows show a visibly more prominent τ band near the top surface than fresh. |
| `09_l8_matrix_relerr_heatmap.png` | Nondimensionalized \|Δu\|/U0, \|Δτ\|/(ρU0²), single settled instant, for the 4 pairwise comparisons that isolate each factor of the MPI/OpenMP x fresh/restart design twice over: row 1/2 = MPI vs OpenMP (fresh/restart respectively, same absolute t since they share the same clock); row 3/4 = fresh vs restart (MPI/OpenMP respectively, phase-matched on the SETTLED tail only -- t mod T_per, restricted to after the ramp completes). Rows 1-2 are essentially zero; rows 3-4 show a real, substantial \|Δu\| difference concentrated in the bulk. |
| `09_l8_matrix_relerr_video.mp4` | Same 4 comparisons, animated across fresh-MPI's full settled tail (168 frames, matching the temporal richness of `08`'s video). Confirms the static instant isn't a fluke: rows 1-2 stay small and rows 3-4 stay large throughout. |
