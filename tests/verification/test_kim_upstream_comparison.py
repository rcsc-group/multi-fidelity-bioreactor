...
Note: like the rest of tests/verification/'s `hpc`-marked tests, this is
NOT part of automated GitHub Actions CI (cloud-hosted, no access to
OSCAR's persistent Basilisk install or compute nodes). Run manually via
`pytest -m hpc tests/verification/test_kim_upstream_comparison.py` on an
OSCAR compute node.
...
PERSISTENT_QCC = Path("/oscar/data/dharri15/eaguerov/basilisk/src/qcc")
...
_OUR_PARAMS = {**CANONICAL_PARAMS, "run_id": "kim_cmp_our_fork", "fidelity": 6, "t_end": 20.0}
...
def kim_upstream_binary(tmp_path_factory):
    """Compile the vendored Kim upstream fixture with the persistent-storage qcc."""
    if not PERSISTENT_QCC.exists():
        pytest.skip(f"persistent qcc not found at {PERSISTENT_QCC}")
    ...
    basilisk_src = PERSISTENT_QCC.parent
    binary = build_dir / "BioReactor_kim_upstream"
    result = subprocess.run(
        [str(PERSISTENT_QCC), "-O2", "-Wall", "-disable-dimensions",
