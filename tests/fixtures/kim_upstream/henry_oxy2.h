...
event defaults (i = 0)
{
  for (scalar s in stracers) {
    ...
    set_prolongation (s, refine_embed_linear);  // COMPAT PATCH (8d6ae01)
#else
    s.refine  = refine_linear;
#endif
    set_restriction (s, restriction_volume_average);  // COMPAT PATCH (8d6ae01):
    // set_restriction()/set_prolongation() replace direct attribute
    // assignment + s.dirty=true; .dirty was removed from _Attributes in
    // Basilisk >= 2025. No behavior change, API rename only.
  }
}
#endif // TREE
...
event tracer_diffusion (i++)
{
  ...
  for (scalar c in stracers) {
    ...
    q.D = D;
    q.beta = beta;
    q.embed_flux = NULL; // NECESSARY FIX (not upstream): uninitialized stack
                         // garbage here otherwise skips the symmetry check
                         // ~30 lines below (`if (!q.embed_flux && ...)`) --
                         // crashes immediately (SIGFPE via div-by-near-zero
                         // in embed_flux, or SIGSEGV) on ANY run, fresh-start
                         // included, under the current Basilisk install.
                         // Not a physics change: sets a struct field upstream
                         // never initializes to the value the very next use
                         // of it already assumes ("if not yet set").
