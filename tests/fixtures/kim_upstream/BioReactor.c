...
const double t_end   = 24.0;     // TEST TRUNCATION ONLY (was 250.0): does not
                                  // alter the simulated equations/dynamics at
                                  // all, only how long the run continues past
                                  // the point needed for the t/T_p=29-31
                                  // comparison window this test targets.
...
const int    i_norm  = 10;      // SAMPLING-ONLY CHANGE (was 1000): 1000
                                // iterations corresponds to ~2 non-dim time
                                // units (~3.3 rocking periods) between
                                // samples -- far too coarse to resolve the
                                // oscillating ux_rms(t) curve without severe
                                // phase-aliasing (same problem this project
                                // found and fixed for its own t_out; see
                                // diary.md). Does not change any simulated
                                // field, timestep, or equation -- only how
                                // often the already-computed statistics are
                                // written to normf.dat.
...
int main(int argc, char * argv[]){
  ...
  L0 = 1. [0];  // COMPAT PATCH (project commit 8d6ae01): [0] dimensional
                // annotation required by current Basilisk's dimensional
                // analysis; upstream's own value (1.0, via LL) unchanged.
  DT = HUGE [0]; // COMPAT PATCH: same reason; HUGE is upstream's own
                 // (implicit, unset) default -- unbounded timestep cap.
