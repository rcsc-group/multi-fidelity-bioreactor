...
| `09_l8_matrix_relerr_heatmap.png` | Nondimensionalized \|Δu\|/U0, \|Δτ\|/(ρU0²) for the 4 pairwise comparisons that isolate each factor of the MPI/OpenMP x fresh/restart design twice over: row 1/2 = MPI vs OpenMP (fresh/restart respectively, same absolute t); row 3/4 = fresh vs restart (MPI/OpenMP respectively, phase-matched on the SETTLED tail only -- t mod T_per, restricted to after the ramp completes, since the flow locks to absolute-t phase, not "time since restart", and an unrestricted phase search can land mid-ramp). Rows 1-2 are essentially zero; rows 3-4 show a real, substantial \|Δu\| difference concentrated in the bulk, confirming the restart-vs-fresh effect is real and not an MPI/OpenMP artifact. |
